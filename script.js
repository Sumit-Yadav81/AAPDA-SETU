...
const stateCoordinates = {
    ...
    "National Capital Territory of Delhi": { lat: 28.7041, lon: 77.1025 },
    "Goa": { lat: 15.4909, lon: 73.8278 },
    "Gujarat": { lat: 22.2587, lon: 71.1924 },
    "Haryana": { lat: 29.0588, lon: 76.0856 },
    "Himachal Pradesh": { lat: 31.1048, lon: 77.1734 },
    "Jammu and Kashmir": { lat: 33.7782, lon: 76.5762 },
    "Jharkhand": { lat: 23.6345, lon: 85.3847 },
...
function setupEventListeners() {
    ...
    // --- Find this in your setupEventListeners function ---
    document.getElementById('authModal').addEventListener('click', (e) => { if (e.target.id === 'authModal') hideAuthModal(); });

// --- Add this new code right below it ---
    const classInputWrapper = document.getElementById('studying-class-wrapper');
    document.querySelectorAll('.role-selector .role-button').forEach(button => {
        button.addEventListener('click', () => {
            // Deselect sibling buttons
            button.parentElement.querySelectorAll('.role-button').forEach(btn => btn.classList.remove('active'));
            // Select clicked button
            button.classList.add('active');

            const selectedRole = button.dataset.role;

            // Check if we are in the sign-up form
            if (button.closest('.sign-up-container')) {
...
function switchView(viewToShow) {
    ...
    if (viewToShow === 'map-view') {
        ...
        // 1. Reset scroll position to the top for the map view.
        // This prevents the view from appearing scrolled down.
        window.scrollTo(0, 0);

        // 2. Force the map to resize after its container becomes visible.
        // We check if mapChart exists and use a tiny delay (10ms) to ensure the
        // container is fully rendered before we ask the map to resize itself.
        if (mapChart) {
            setTimeout(() => {
                mapChart.resize();
            }, 10); 
...
// NEW FUNCTION TO SIMULATE FETCHING API DATA
// async function getAlertForState(stateName) {
//     console.log(`Fetching alert for ${stateName}...`);
    
//     // This simulates a network delay, like a real API call would have.
//     await new Promise(resolve => setTimeout(resolve, 500)); 

//     // For now, we return your mock data.
//     // In the future, a real 'fetch()' call would go here.
//     const alertData = mockAlerts[stateName] || mockAlerts["Default"];
//     return alertData;
// }
async function getAlertForState(stateName) {
    ...
    if (!coords) {
        return mockAlerts["Default"]; 
    }

    const { lat, lon } = coords;
    const apiUrl = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=precipitation&forecast_days=1`;
    ...
    try {
        ...
        // --- New Multi-Level Alert Logic ---
        const upcomingPrecipitation = data.hourly.precipitation.slice(0, 12);
        
        // 1. Check for Red Alert condition (Extreme Rain > 7.5 mm/hr)
        const isExtremeRainForecast = upcomingPrecipitation.some(p => p > 7.5);
        if (isExtremeRainForecast) {
            return {
                level: "Red",
                disasterType: "Flash Floods",
                message: `DANGEROUS: Extreme rainfall forecast for ${stateName}. Risk of flash floods is high. Take immediate precautions.`,
                time: new Date().toLocaleTimeString('en-IN', { hour: 'numeric', minute: '2-digit' })
            };
        }

        // 2. Check for Orange Alert condition (Heavy Rain > 0.5 mm/hr for our test)
        const isHeavyRainForecast = upcomingPrecipitation.some(p => p > 0.5);
        if (isHeavyRainForecast) {
            return {
                level: "Orange",
                disasterType: "Floods",
                message: `Heavy rainfall is forecast for ${stateName} within the next 12 hours. Be aware of potential for flooding.`,
                time: new Date().toLocaleTimeString('en-IN', { hour: 'numeric', minute: '2-digit' })
            };
        }

        // 3. If no severe weather, return the Green alert
        return mockAlerts["Default"];
...
async function updateAlertButton() {
    const state = userProgress.selectedState || "Default";
    const alertData = await getAlertForState(state); // Use the new async function
...
function checkCertificateEligibility() {
    ...
    if (completedBadges >= totalModules) {
        ...
        const maxQuizPoints = Object.values(learningModules).reduce((sum, module) => sum + module.points, 0);
        const maxRevisionPoints = Object.values(stateDisasters).flatMap(s => s.primaryHazards).filter((v, i, a) => a.indexOf(v) === i).length * 5;
        const maxTotalPoints = maxQuizPoints + maxRevisionPoints;

        let userTier = 'bronze'; // Default tier
        if (userProgress.points === maxTotalPoints) {
            userTier = 'gold';
        } else if (userProgress.points >= 400) { // Threshold for Silver
            userTier = 'silver';
        }

        const certButton = document.createElement('button');
        certButton.textContent = 'Download Your Certificate!';
        certButton.className = 'certificate-button';
        // Pass the determined tier to the generate function
        certButton.onclick = () => generateCertificate(userTier);
        container.appendChild(certButton);
    }
}
async function generateCertificate(tier) {
    const canvas = document.createElement('canvas');
    canvas.width = 1000;
    canvas.height = 707;
    const ctx = canvas.getContext('2d');

    // Define properties for each certificate tier
    ...
    const template = certificateTemplates[tier];
    const image = new Image();

    // vvv THIS IS THE FIX vvv
    image.crossOrigin = "Anonymous"; // Add this line before setting the image.src

    image.src = template.src;

    image.onload = function() {
        // 1. Draw the correct certificate background
        ...
        ctx.textAlign = 'center';
        ctx.fillText(userName, canvas.width / 2, template.nameY);

        // 3. Draw the issued date
        const today = new Date();
        const dateString = today.toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
        ctx.fillStyle = template.nameColor;
        ctx.font = '20px Inter, sans-serif';
        ctx.textAlign = 'left';
        // Use the new dateX property for correct horizontal placement
        ctx.fillText(dateString, template.dateX, template.dateY);

        // 4. Trigger the download
        const link = document.createElement('a');
        link.download = `DisasterMaster_Certificate_${userName.replace(' ','_')}.png`;
        link.href = canvas.toDataURL('image/png');
        link.click();
    };

    image.onerror = function() {
        alert("Could not load certificate template. Please ensure image files are correctly named and in the same folder.");
    }
}
